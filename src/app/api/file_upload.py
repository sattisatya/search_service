...
async def upload_file(file: UploadFile = File(...), chat_id: Optional[str] = None):
    ...
    try:
        ...
        update_chat_meta_on_message(chat_id, "question", title=f"Upload: {file.filename}", document_ids=[doc_id])
        update_chat_order("question", chat_id)
    except Exception:
        pass
...
#     # chat_type_final = request.chat_type or "documentqna"
#     # # allow 'upload' as a valid chat_type so we can store/retrieve doc meta there
#     # if chat_type_final not in ("documentqna", "question", "insight", "upload"):
